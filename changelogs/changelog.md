...
- 「启用终端输入」调试开关默认改为开启，新生成的配置可直接在交互式终端发送消息和执行 `/clear`、`/pm` 等指令；已有配置的取值不受影响（非交互式 stdin 下仍会自动跳过）。
- 修复回复分割数超过 `max_split_num` 被压缩合并回一条消息时丢失分隔符，导致多句粘连成无标点长文本的问题，拼接时补回句间分隔符。
- 移除已无调用的旧分句合并函数 `merge_sentences_to_max_count`。
- 修复统计任务的指标趋势与图表数据全量加载消息实体导致内存周期性暴增的问题，消息查询改为仅投影统计所需的 6 列。
- 修复适配器黑白名单面板与运行时策略求值身份标准不一致的问题：面板读写改用运行中适配器的完整身份并按 specificity 定位既有条目，不再读出空模板或写出被更高优先级条目遮蔽的死规则；适配器未运行时保存会被明确拒绝。
- 统一模型服务商的超时与重试默认为后端单一口径（timeout 120 秒、max_retry 3、retry_interval 4）：WebUI 不再预填或回填 30/2/10，新用户默认配置模板同步对齐，避免前后端默认值分叉。

## 插件

- 统计插件快照历史审计遇到服务端索引有条目、但文件实际已 404 丢失时，记录并跳过该文件继续补齐其余缺口；先精确请求缺口起始文件、成功后才用 `next_n` 批量跟进，避免为每个已丢失文件重复付出批量加载开销，修复单个服务端丢失文件中断整轮审计并周期性刷错误日志的问题。

## Maisaka

- 移除实验性三维建模工具、模型导入编辑与预览渲染能力及其配置开关，保留通用图片上下文与发图功能。
...
- 聊天管理详情弹窗：Session ID 与平台、类型、ID 合并为一行；「适配器放行」更名为「适配器规则」并移除冗余说明；已允许聊天的重复提示文案不再展示。
- 适配器设置：群聊/私聊的全局默认策略移至适配器设置页顶部统一编辑，聊天流设置内改为只读一行展示；侧边栏「适配器管理」更名为「适配器设置」。
...
- 插件市场新增兼容性过滤提示：被默认开启的「只看兼容」挡掉的插件会显示隐藏数量与当前麦麦版本，并提供跳转插件设置页关闭该开关的入口，避免用户因老插件全部消失而误以为插件已下架（例如搜索「联网」时只剩支持当前版本的现代插件）。
...
- 缓存计价简化为按缓存单价填写与否决定：移除「支持缓存」开关，缓存价格输入恒显示，留空时缓存命中与非缓存输入一致；旧 `cache` 字段保留兼容但不再参与计价。
- 模型配置页思考开关按服务商模板适配：通用开关覆盖智谱/Kimi/MiniMax/豆包（thinking.type）、百炼/硅基流动（enable_thinking）、阶跃/OpenAI/xAI（reasoning_effort 调档，思考常开并给出说明），豆包与 Qwen 支持思考预算；DeepSeek 保留专用开关（含 Responses 客户端与联网搜索）。新增智谱编程套餐（GLM Coding Plan）、阶跃 Step Plan、火山方舟编程套餐（Ark Coding Plan）服务商模板。
